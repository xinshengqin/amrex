module advect_cuda_module
    implicit none
    contains
subroutine advect_cuda(time, lo, hi, &
     &            uin , ui_lo, ui_hi, &
     &            uout, uo_lo, uo_hi, &
     &            vx  , vx_lo, vx_hi, &
     &            vy  , vy_lo, vy_hi, &
  ...
  integer :: i, j
  integer :: glo(2), ghi(2)
  double precision :: dtdx_x, dtdx_y, dx_x, dx_y
  ...
  double precision, device, allocatable :: phix_1d(:,:), phiy_1d(:,:), phix(:,:), phiy(:,:), slope(:,:)
  ...
  allocate(phix_1d( glo(1): ghi(1), glo(2): ghi(2)))
  allocate(phiy_1d( glo(1): ghi(1), glo(2): ghi(2)))
  allocate(phix   ( glo(1): ghi(1), glo(2): ghi(2)))
  allocate(phiy   ( glo(1): ghi(1), glo(2): ghi(2)))
  allocate(slope  ( glo(1): ghi(1), glo(2): ghi(2)))
  ...
  ! TODO: get device version of this
  ...
  do    j = lo(2),hi(2)
     do i = lo(1),hi(1)
        uout(i,j) = uin(i,j) + &
             ( (flxx(i,j) - flxx(i+1,j)) * dtdx_x &
             + (flxy(i,j) - flxy(i,j+1)) * dtdx_y )
     enddo
  enddo
...
end subroutine advect_cuda
end module advect_cuda_module
