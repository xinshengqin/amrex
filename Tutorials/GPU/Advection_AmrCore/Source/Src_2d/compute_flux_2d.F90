  ...
  subroutine compute_flux_2d(lo, hi, dt, dx, &
    ...
    do    j = lo(2)  , hi(2)+1
       do i = lo(1)-1, hi(1)+1
          ...
          if (vmac_d(i,j) .lt. 0.d0) then
             phiy_1d_d(i,j) = phi_d(i,j  ) - (0.5d0 + hdtdx_y*vmac_d(i,j))*slope_d(i,j  )
          else
             phiy_1d_d(i,j) = phi_d(i,j-1) + (0.5d0 - hdtdx_y*vmac_d(i,j))*slope_d(i,j-1)
          end if
    ...
    do    j = lo(2), hi(2)
       do i = lo(1), hi(1)+1
          ...
          if (umac_d(i,j) .lt. 0.d0) then
             phix_d(i,j) = phix_1d_d(i,j) &
                  - hdtdx_y*( 0.5d0*(vmac_d(i  ,j+1)+vmac_d(i  ,j)) * (phiy_1d_d(i  ,j+1)-phiy_1d_d(i  ,j)) )
          else
             phix_d(i,j) = phix_1d_d(i,j) &
                  - hdtdx_y*( 0.5d0*(vmac_d(i-1,j+1)+vmac_d(i-1,j)) * (phiy_1d_d(i-1,j+1)-phiy_1d_d(i-1,j)) )
          end if

          ! compute final x-fluxes
          flxx_d(i,j) = phix_d(i,j)*umac_d(i,j)
    ...
    do    j = lo(2), hi(2)+1
       do i = lo(1), hi(1)
          ...
          if (vmac_d(i,j) .lt. 0.d0) then
             phiy_d(i,j) = phiy_1d_d(i,j) &
                  - hdtdx_x*( 0.5d0*(umac_d(i+1,j  )+umac_d(i,j  )) * (phix_1d_d(i+1,j  )-phix_1d_d(i,j  )) )
          else
             phiy_d(i,j) = phiy_1d_d(i,j) &
                  - hdtdx_x*( 0.5d0*(umac_d(i+1,j-1)+umac_d(i,j-1)) * (phix_1d_d(i+1,j-1)-phix_1d_d(i,j-1)) )
          end if

          ! compute final y-fluxes
          flxy_d(i,j) = phiy_d(i,j)*vmac_d(i,j)
